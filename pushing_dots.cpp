// буксир.cpp : Defines the entry point for the application.
#include "stdafx.h"
#include "буксир.h"
#include <fstream>
#include <commdlg.h>
...
{   HWND hWnd;  hInst = hInstance; // Store instance handle in our global variable
   hWnd = CreateWindow(szWindowClass, szTitle, WS_OVERLAPPEDWINDOW,
      CW_USEDEFAULT, 0, CW_USEDEFAULT, 0, NULL, NULL, hInstance, NULL);if (!hWnd){ return FALSE; } ShowWindow(hWnd,nCmdShow);UpdateWindow(hWnd);   return TRUE;}
const int MARK = 6;//разм кубиков
...
void DcInLp(POINT &point)//ф-я д/преобразования коорд в лог
{	point.x = point.x* SCALE / sx;
	point.y = SCALE - point.y* SCALE / sy;}
void transform(HDC& hdc)//переход влогич коорд
...
{	PAINTSTRUCT ps;
	HDC hdc;
	TCHAR s[20];
	static TCHAR nameG1[] = _T("1.0");	static TCHAR nameG2[] =_T("2.0");
	static TCHAR nameG3[] =_T("3.0");	static TCHAR nameG4[] =_T("4.0");
	static TCHAR nameG5[] =_T("5.0");	static HPEN hDash, hBezier;
	static HBRUSH hRect, hSel;static POINT pt[20];	static POINT point;RECT rt;static POINT *pt1;	static int count, index;static bool capture;
	int i;	std::ifstream in;std::ofstream out;
	static double max_x, max_y, min_x, min_y;
	int  x, y;DOUDLE_POINT t;double z, hx, hy;	static DOUDLE_POINT *xy;
	switch (message)
	{	case WM_CREATE:
		in.open("dat.txt");
		if (in.fail())
	...
	case WM_PAINT:
		...
		for (z = min_x, i = 0; i <= scaleX; z += hx, i++)
		{
			...
			TextOut(hdc, 27, 60, s, _tcslen(s));
			MoveToEx(hdc,40,30, NULL);		
LineTo(hdc, 40, 700);
		}
...
TextOut(hdc, 150, 8, nameG1, _tcslen(nameG1));
TextOut(hdc, 250, 8, nameG2, _tcslen(nameG2));
TextOut(hdc, 350, 8, nameG3, _tcslen(nameG3));
TextOut(hdc, 450, 8, nameG4, _tcslen(nameG4));

TextOut(hdc, 27, 150, nameG1, _tcslen(nameG1));
TextOut(hdc, 27, 250, nameG2, _tcslen(nameG2));
TextOut(hdc, 27, 350, nameG3, _tcslen(nameG3));
TextOut(hdc,27 , 450, nameG4, _tcslen(nameG4));
TextOut(hdc,27 , 550, nameG5, _tcslen(nameG5));
		MoveToEx(hdc, 0, 0, NULL);
		LineTo(hdc, 0, SCALE);
